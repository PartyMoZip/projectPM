				...
				<div class="table-Detail">
					...
					<div class="content">
						...
						<div class="info_desc">
							...
							<div class="cover_info">
								<div>${boardDetail.nickname}<br> <span>${boardDetail.qdate}</span>
					...
					<div class="comment_area">
										...
										<c:forEach items="${__COMMENT__}" var="comment">
											...
											<form action="/qnaboard/editComment" method="post">
												...
												<input type="hidden" name="reCurrPage"
													...
													name="qrefer" value="${comment.qrefer}"> <input
													type="hidden" name="qrerefer" value="${comment.qrerefer}">

												<div class="qnaBoardCommentInfo">
													<div class="qnaBoardCommentWriter">
														<c:out value="${comment.nickname}" />
													</div>
													&nbsp;&nbsp;
													<div>
														<fmt:formatDate pattern="yyyy.MM.dd HH:mm:ss"
															value="${comment.qredate}" />
													</div>
												</div>

												<div class="qnaBoardCommentContent">
													<c:out value="${comment.qrecontent}" />
												</div>

												<div class="qnaBoardCommentEdit">
													<input type="hidden" id="${comment.qrerefer}totext"
														name="qrecontent" value="${comment.qrecontent}"> <input
														type="hidden" name="email"
														value="${sessionScope.__AUTH__.email}">
													<c:if
														test="${sessionScope.__AUTH__.email eq comment.email}">
														<input type="button" id="${comment.qrerefer}" name="mod"
															value="수정">
														<input type="hidden" id="${comment.qrerefer}tosubmit"
															value="수정완료">
													</c:if>
												</div>
											...
											<c:if test="${sessionScope.__AUTH__.email eq comment.email}">
												<form action="/qnaboard/deleteComment" method="post">
													<input type="hidden" name="currPage"
														...
														type="hidden" name="qrefer" value="${comment.qrefer}">
													<input type="hidden" name="qrerefer"
														value="${comment.qrerefer}"> <input type="submit"
														value="삭제">
												</form>
											</c:if>
										...
										<ul class="pagination justify-content-center">
											<c:if test="${replyPageMaker.prev}">
												<li class="prev page-item"><a class="prev page-link"
													href="/qnaboard/showQnaDetail?qrefer=${boardDetail.qrefer}&currPage=${cri.currPage}&reCurrPage=${replyPageMaker.startPage-1}">◀</a>
												</li>
											</c:if>
											<c:forEach begin="${replyPageMaker.startPage}"
												...
												<li class="page-item"><a id="page-curr"
													class="page-link"
													href="/qnaboard/showQnaDetail?qrefer=${boardDetail.qrefer}&currPage=${cri.currPage}&reCurrPage=${pageNum}">
														${pageNum} </a></li>
											</c:forEach>

											<c:if test="${replyPageMaker.next}">
												<li class="next page-item"><a class="next page-link"
													href="/qnaboard/showQnaDetail?qrefer=${boardDetail.qrefer}&currPage=${cri.currPage}&reCurrPage=${replyPageMaker.endPage+1}">▶</a>
												</li>
											</c:if>
										</ul>
									</form>
						...
						<form action="/qnaboard/writeComment" method="POST">
							...
							<input type="hidden" name="reCurrPage"
								value="${recri.reCurrPage}"> <input type="hidden"
								name="qrefer" value="${boardDetail.qrefer}"> <input
								type="hidden" name="email"
								value="${sessionScope.__AUTH__.email}">
					...
					<div class="container-btnGroup d-flex justify-content-end">
						...
						<c:if test="${boardDetail.nickname eq nickname}">
							...
							<form action="/qnaboard/deleteQnaBoard" name="deleteButton"
								method="post">
								<button type="submit" id="deleteButton"
									class="btn btn-primary btn-sm">
									<input type="hidden" id="qrefer" name="qrefer"
										value="${boardDetail.qrefer}"> <i
